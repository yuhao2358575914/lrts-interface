...
def bookName():
    '''生成付费收听版权方全称'''
    cp_record = billing_select("select * from platform.t_copyright  order by id desc limit 1;","platform")
...
def readBookName():
    '''生成电子阅读版权方全称'''
    cp_record = billing_select("select * from readbook.rb_partner_ext order by partner_id desc limit 1;","readbook")
