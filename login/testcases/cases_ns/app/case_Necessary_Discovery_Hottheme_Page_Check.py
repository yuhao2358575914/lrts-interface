...
class case_Necessary_Discovery_Hottheme_Page_Check(unittest.TestCase):

    def test_Discovery_Hottheme_Page_Check(self):
        """发现页-热门话题，数据正常返回"""
        # 登录并获取token
        ...
        # 首次请求资源筛选页数据
        r = httputils.get_app(getconf.get_global_conf('apinames', 'themeHotList'), data)
        print(r.text)
        list2 = json.loads(r.text)['list']
        self.assertTrue(len(list2) > 3)
